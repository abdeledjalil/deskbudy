...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (email.isEmpty || password.length < 6) {
      setState(() =>
          _errorMessage = 'أدخل بريدًا صحيحًا وكلمة مرور من 6 أحرف على الأقل');
      return;
    }
  ...
  @override
  Widget build(BuildContext context) {
    final title = _isRegistering ? 'إنشاء حساب' : 'تسجيل الدخول';
    return Scaffold(
      appBar: AppBar(title: const Text('DeskBuddy')),
      body: Center(
        child: SingleChildScrollView(
          ...
          child: ConstrainedBox(
            ...
            child: Column(
              ...
              children: [
                const Icon(Icons.medication, size: 72, color: Colors.indigo),
                ...
                ElevatedButton(
                  ...
                  child:
                      _isBusy ? const CircularProgressIndicator() : Text(title),
                ),
                TextButton(
                  onPressed: _isBusy
                      ? null
